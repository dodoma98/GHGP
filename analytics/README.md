...
```
판매 페이지 ┐
홈페이지    ├→ Cloudflare Worker → D1 데이터베이스
포인트 사이트 ┘                          ↑
                        관리자 → 대시보드 (비밀번호 로그인)
```
...
| `SITE_NAMES` | Text | 사이트 코드와 표시 이름<br>예: `ghgp=판매 페이지,home=홈페이지,point=포인트` | 선택 |
...
- `SITE` 만 사이트마다 다르게: 판매 페이지 `ghgp`, 홈페이지 `home`, 포인트 사이트 `point` 등 (영문 소문자 권장)
